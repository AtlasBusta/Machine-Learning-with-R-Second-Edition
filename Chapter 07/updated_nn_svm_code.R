...
# Build the model with a cost value that provides the greatest overall accuracy value

# Register parallel back-end
registerDoParallel(cores = (detectCores() - 1))

set.seed(12345)
letter_classifier_rbf <- ksvm(letter ~ ., data = letters_train, kernel = "rbfdot", C = 25)
letter_predictions_rbf <- predict(letter_classifier_rbf, letters_test)
